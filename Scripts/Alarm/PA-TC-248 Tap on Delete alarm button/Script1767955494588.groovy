...
TestObject deleteAlarmBtn = findTestObject('Object Repository/Test_3/android.widget.Button - Delete alarm')
TestObject removeAlarmBtn = findTestObject('Object Repository/Test_3/android.widget.Button - Remove alarm')
TestObject alarmsText     = findTestObject('Object Repository/Test_3/android.widget.TextView - Alarms')
...
// Step 5: If alarms exist, delete
if (isAlarmVisible) {
	Mobile.tap(alarmLayout, 10)
	KeywordUtil.logInfo('Tapped existing alarm layout')

	Mobile.tap(deleteAlarmBtn, 0)
	KeywordUtil.logInfo('Tapped Delete Alarm button')

	Mobile.tap(removeAlarmBtn, 0)
	KeywordUtil.logInfo('Confirmed removal by tapping Remove Alarm button')

	try {
		Mobile.verifyElementVisible(alarmsText, 5, FailureHandling.STOP_ON_FAILURE)
		KeywordUtil.logInfo('"Alarms" screen verified successfully after deletion')
	} catch (Exception e) {
		KeywordUtil.logInfo('Failed to verify "Alarms" screen after deletion: ' + e.message)
	}

	Mobile.takeScreenshot()
	KeywordUtil.logInfo('Screenshot captured after deleting alarm')
} else {
